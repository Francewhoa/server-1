...
static int write_compressed(File fd,  uchar *data, size_t len, size_t pagesize)
{
	...
	for (size_t written= 0; written < len;)
	{
		...
		if (datasize < n_bytes) {
			...
			if (my_seek(fd, n_bytes - datasize, MY_SEEK_CUR, MYF(MY_WME | MY_NABP))
				== MY_FILEPOS_ERROR)
			 return 1;
		}
		written += n_bytes;
...
{
	my_off_t pos= my_seek(fd, 0, MY_SEEK_CUR, MYF(MY_WME | MY_NABP));
	if (pos == MY_FILEPOS_ERROR)
		return -1;
	return my_chsize(fd, pos , 0, MYF(MY_WME | MY_NABP));
}
